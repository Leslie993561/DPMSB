...
import { converterVariaveisImportadas } from "@/lib/parsing/variaveisBeneficios";
import { importarVariaveis } from "@/lib/db/beneficiosVariaveis";

export const runtime = "nodejs";

const schema = z.object({ competencia: z.string().regex(/^\d{4}-\d{2}$/) });

/** Lê a planilha de variáveis (Transporte/Mobilidade/Alimentação avulsos) e SOMA na competência informada — nunca sobrescreve. */
