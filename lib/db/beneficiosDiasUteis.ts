 ...
 */
export async function obterDiasUteis(ano: number, mes: number): Promise<number> {
  const db = await getDb();
  const resultado = await db.execute({
    sql: `SELECT dias_uteis FROM beneficios_dias_uteis
       WHERE ano < ? OR (ano = ? AND mes <= ?)
       ORDER BY ano DESC, mes DESC
       LIMIT 1`,
    args: [ano, ano, mes],
  });
  const linha = resultado.rows[0] as unknown as { dias_uteis: number } | undefined;
 ...
 */
export async function listarDiasUteisAno(
  ano: number,
): Promise<{ mes: number; diasUteis: number; origem: "ajustado" | "herdado" | "padrao" }[]> {
  const db = await getDb();
  const resultado = await db.execute({ sql: "SELECT * FROM beneficios_dias_uteis WHERE ano = ?", args: [ano] });
  const linhasAno = resultado.rows as unknown as LinhaDiasUteis[];
  ...
  return Promise.all(
    Array.from({ length: 12 }, async (_, i) => {
      ...
      const efetivo = await obterDiasUteis(ano, mes);
...
export async function definirDiasUteis(ano: number, mes: number, diasUteis: number): Promise<void> {
  const db = await getDb();
  await db.execute({
    sql: `INSERT INTO beneficios_dias_uteis (ano, mes, dias_uteis) VALUES (?, ?, ?)
       ON CONFLICT(ano, mes) DO UPDATE SET dias_uteis = excluded.dias_uteis`,
    args: [ano, mes, diasUteis],
  });
}
