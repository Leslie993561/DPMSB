 ...
 */
export function importarProgramacaoAnual(
  itens: LinhaProgramacaoAnual[],
  nomeArquivo: string,
): ResultadoProgramacaoAnual {
  const db = getDb();
  ...
  itens.forEach((item, indice) => {
    ...
    if (!periodo) {
      const info = db
        .prepare(
          `INSERT INTO periodos_aquisitivos (colaborador_id, data_inicio, data_fim, dias_direito)
           VALUES (?, ?, ?, ?)`,
        )
        .run(colaborador.id, item.aquisitivoInicio, item.aquisitivoFim, DIAS_DIREITO_PADRAO);
    ...
    db.prepare("DELETE FROM lancamentos_ferias WHERE periodo_aquisitivo_id = ? AND observacao LIKE ?").run(
      periodo.id,
      `%${marcador}`,
    );
    ...
    ).run(periodo.id, item.diasFerias, item.inicioFerias, item.abono ? 1 : 0, item.diasAbono, observacao);
    lancados++;
  });
